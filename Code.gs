 ...
 *     서비스를 별도로 활용신청해야 합니다 (승인되면 위 키 그대로 같이 씁니다).
 *     - 출몰시각정보(getAreaRiseSetInfo), 음력 정보(getLunCalInfo) 둘 다 신청 필요.
 * ============================================================
 */
...
// ============================================================
//  5) 일출/일몰/월출/월몰 (한국천문연구원 - 출몰시각정보, getAreaRiseSetInfo)
//     ⚠️ location 파라미터는 KASI가 정한 "지역 코드"입니다 (위경도가 아님).
//        정확한 코드는 같은 서비스의 "지역구분조회(getAreaCodeInfo)"에서 확인하세요.
//        아래 41은 "제주" 지역으로 흔히 쓰이는 코드를 임시로 넣어둔 것이니,
//        실제 응답의 지역명(location)이 제주가 맞는지 꼭 확인 후 필요시 수정하세요.
// ============================================================
function handleAstro(date) {
  var reqDate = date || Utilities.formatDate(new Date(), 'Asia/Seoul', 'yyyyMMdd');
  var locationCode = 41; // ⚠️ 제주 지역코드 - 실제 응답 확인 후 필요시 조정

  var url = 'https://apis.data.go.kr/B090041/openapi/service/RiseSetInfoService/getAreaRiseSetInfo'
    + '?serviceKey=' + encKey()
    + '&locdate=' + reqDate
  ...
  // 응답 시각 형식은 보통 "HHmm" 4자리 문자열(예: "0513") - 화면 표시용으로 HH:mm 변환
  var fmt = function (v) {
